 ...
 * @param [req.body.password] {String} User's signin password (used only for
 *   authentication methods that need it, like OIDC)
...
IdentityProvider.prototype.post = function post (req, res, next) {
  ...
  waterfall([
    (callback) => {
      ...
      const userStore = req.app.locals.oidc.users
      if (!userStore) {
        let error = new Error('No OIDC user store configured')
        error.status = 500
        return callback(error)
      }
      return userStore.createUser(webId, options)
        .then(() => callback())
        .catch((err) => {
          callback(err)
        })
